...
def save_backup_list(backup_list, filename="log/list_new_course-from-v4-M.pkl"):
    with open(filename, "wb") as file:
        pickle.dump(backup_list, file)


def load_backup_list(filename="log/list_new_course-from-v4-M.pkl"):
    try:
...
async def fetch_sikola_course_users():
    async with aiohttp.ClientSession() as session:
        ...
        for filePath in listDataDetailKelasFileOldVer:
            ...
            newFilePath = (
                f"data/detailkelas/{kelasActiveNameNewVer}/{splitOldPathFileName}"
            )
        ...
        for filePath in listDataDetailKelasFileNewVer:
            ...
            oldFilePath = (
                f"data/detailkelas/{kelasActiveNameOldVer}/{splitNewPathFileName}"
            )
        ...
        with open(
            f"data/detailkelas/ChangeItem/mahasiswa/{kelasActiveNameNewVer}.json", "w"
        ) as json_file:
