...
async def fetch_sikola_course():
    async with aiohttp.ClientSession() as session:
        active_semesters = ["TA241", "TA232"]  # Daftar semester aktif
        kwu_keywords = ["KEWIRAUSAHAAN", "ENTERPRENEUR", "KWU"]
        backup_file = "backup_course_data_kwu-5.pkl"

        # Load progress from backup
        backup_data = load_backup(backup_file) or {"processed_prodi": set(), "data_reports": []}
        ...
        for prodi in datafakultasProdi["prodis"]:
            ...
            # Filter courses by active semesters and keywords
            filtered_courses = [
                course for course in result_course["courses"]
                if any(semester in course["shortname"] for semester in active_semesters) and
                   any(kwu in course["fullname"].upper() for kwu in kwu_keywords)
            ]
        ...
        df.to_excel("course_reports_kwu_5.xlsx", index=False)
