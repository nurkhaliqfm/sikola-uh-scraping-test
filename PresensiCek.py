...
async def fetch_sikola_course(kelasActiveName):
    backup_file = "backupCekPresensi-3.json"
    excel_file = "presensi_cek_20-02-2.xlsx"
...
async def course_reports(
 ...
 # Tambahkan data ke dalam laporan
            dataReports.append([
                ...
                fakultas,   
                jumlahSession,  
                f"Ada Activity Presensi pada section {section_name}",  
                attendanceUrl,
                kelas_id
            ])
        else:
            # Jika tidak ada module attendance, tambahkan informasi course tanpa attendance
            dataReports.append([
                course_id,
                namaKelas,
                namaProdi,
                fakultas,
                0,  # Tidak ada session
                "Tidak Ada Activity Presensi",  # Keterangan absence of attendance
                f'https://sikola-v2.unhas.ac.id/course/view.php?id={course_id}',
                kelas_id

            ])
            
            # os.makedirs(f"data/cekPresensi/{kelasActiveName}", exist_ok=True)

            # with open(f'data/cekPresensi/{kelasActiveName}/{kelas_id}.json', 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    listDataDetailKelasFile = glob.glob(f"data/cekPresensi/{kelasActiveName}/*.json")
